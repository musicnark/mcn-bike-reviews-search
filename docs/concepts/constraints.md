# Archived Prototype Constraints
...
The URLs are also pre-defined in a pre-formatted CSV file, which was manually downloaded from MCN's organisation board (Monday.com) and manually formatted. Additions to the code that integrate with the Monday.com API and automatically format the CSV contents would be ideal, but weren't practical to achieve the desired pace of development. A different algorithm could be used to scrape the live site, but legal implications related to web scraping would have to be considered.
...
This tool also relies on the live site not changing its formatting, which is inherently fragile. This is mitigated by my personal experience with the site, as it hasn't changed its front-end format for as long as I've worked at MCN, but if it was changed an update to the code would be needed to retain functionality.
