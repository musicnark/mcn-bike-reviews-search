...
- `query-bikes` – Filter and sort bike reviews programmatically. Its docstring contains all parameters and return format.
- `pc-extract-specs-from-table` – Extracts bike specs from HTML tables into a structured property list (plist).  
...
#+begin_src sh
mkdir -p ~/.emacs.d/lisp/mcn-bike-reviews-search/ && mv *.el ~/.emacs.d/elisp/mcn-bike-reviews-search/
#+end_src
...
- By default, it will load a local version of the bikes hashmap. If you want to try the download behaviour:
#+begin_src elisp
